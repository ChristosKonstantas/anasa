...
RenderSettings makeTestRenderSettings(int workIterations = 4)
{
...
void initializeJob(RenderJob& job, VersionTable& versions, int chunk)
{
    job.chunk = chunk;
...
TEST_CASE("Renderer: writes only the requested tile")
{
    ...
    for (int frame = 0; frame < CHUNK_FRAMES; ++frame)
    {
        ...
        {
            REQUIRE(job.samples[frame] != UNTOUCHED_SAMPLE);
...
TEST_CASE("Renderer: identical inputs produce identical output")
{
    ...
    initializeJob(firstJob, versions, 1);
    initializeJob(secondJob, versions, 1);
...
TEST_CASE("Renderer: stop request cancels before rendering")
{
    ...
    for (float sample : job.samples)
        REQUIRE(sample == UNTOUCHED_SAMPLE);
}
